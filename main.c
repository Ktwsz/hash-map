...
void init_ht(hash_table *p_table, size_t size, DataFp dump_data, CreateDataFp create_data,
		 DataFp free_data, CompareDataFp compare_data, HashFp hash_function, DataPFp modify_data) {
            ...
            for (size_t i = 0; i < p_table->size; ++i) p_table->ht[i].next = NULL;
...
void dump_list(const hash_table* p_table, size_t n) {
    for (ht_element i = p_table->ht[n]; i.next; i = *(i.next)) p_table->dump_data(i.next->data);
}
...
void free_table(hash_table* p_table) {
    for (size_t i = 0; i < p_table->size; ++i) {
        ...
        while (p.next) {
            ht_element tmp = *p.next;
            free_element(p_table->free_data, p.next);
            p = tmp;
        }
    }
    free(p_table->ht);
...
void rehash(hash_table *p_table) {
	hash_table p_new;
	init_ht(&p_new, p_table->size*2, p_table->dump_data, p_table->create_data, p_table->free_data, p_table->compare_data, p_table->hash_function, p_table->modify_data);

	for (size_t i = 0; i < p_table->size; ++i) {
        ht_element p = p_table->ht[i];
        while (p.next) {
            insert_element(&p_new, &(p.next->data));
			p = *p.next;
        }
    }
	free(p_table->ht);
...
ht_element *find_previous(hash_table *p_table, data_union data) {
    size_t ix = p_table->hash_function(data, p_table->size);
    ht_element* i;
    for (i = &(p_table->ht[ix]); i->next && p_table->compare_data(i->next->data, data); i = i->next);
    return i;
}
...
// insert element
void insert_element(hash_table *p_table, data_union *data) {
	ht_element* e = get_element(p_table, data);
	if (e) {
		if (p_table->modify_data) p_table->modify_data(&(e->data));
		return;
	}
	size_t ix = p_table->hash_function(*data, p_table->size);
    ht_element* el = &(p_table->ht[ix]);

	ht_element* n = el->next;
    el->next = malloc(sizeof(ht_element));
    el->next->data = *data;
    el->next->next = n;
    p_table->no_elements++;
...
void remove_element(hash_table *p_table, data_union data) {
    ht_element* el = find_previous(p_table, data);
	if (!el->next) return;
    ht_element* to_del = el->next;
...
int cmp_word(data_union a, data_union b) {
	DataWord *d1 = (DataWord*)a.ptr_data, *d2 = (DataWord*)b.ptr_data;
...
data_union create_data_word(void *value) {
	char* s = (char*)value;
	data_union d;
	d.ptr_data = malloc(sizeof(DataWord));
	DataWord* dw = (DataWord*)d.ptr_data;
	dw->counter = 1;
	dw->word = malloc(strlen(s));
	strcpy(dw->word, s);
	dw->word = strlwr(dw->word);
	return d;
}
